...
import GeinsAuthClient from './authClient';
...
class AuthManager {
  ...
  async initClient() {
    if (!this.client) {
      this.client = new GeinsAuthClient(
        async (id: string) => await fetch(this.initConfig.signEndpoint + id).then(res => res.text()),
  ...
  update(payload: { credentials?: Credentials; refetchQueries?: boolean } = {}) {
    const credentials = payload?.credentials;
    let refetchQueries = payload?.refetchQueries || false;
    let broadcast = typeof window !== "undefined";

    let username: string | null = credentials ? credentials.username : Cookies.get('geins-user') || null;
    if (this.client?.authorized) {
      ...
      let maxage: number = credentials ? (credentials.rememberUser ? 604800 : 1800) : parseInt(Cookies.get('geins-user-maxage') || '604800', 10);
      
      this.setUser(username);
      Cookies.set('geins-user', username!, { path: '/', expires: maxAgeDays });
      Cookies.set('geins-user-maxage', maxage.toString(), { path: '/', expires: maxAgeDays });
    } else if (this.user !== null) {
      username = null;
      this.clearTokenTimeout();
      this.setUser(username);
      ['geins-auth', 'geins-user', 'geins-user-maxage', 'geins-user-type'].forEach(key =>
    ...
    if (broadcast && this.broadcastChannel) {     
      this.broadcastChannel.postMessage({ type: 'auth', data: username });
    }
