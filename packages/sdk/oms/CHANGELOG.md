...
- 08a2175: Surface `availableChannels` on the user response from `crm.user.get()`.
...
- fd50607: Route `userToken` through every checkout mutation so the Merchant API
...
- 4064242: Route `userToken` through cart mutations so the Merchant API receives the
...
- 52fe9cf: Replace FavoritesSession with unified ListsSession for product lists and favorites management

### Patch Changes

- f4cef4e: Add RequestContext for per-request locale/market overrides
...
- a40809f: Add optional userToken to RequestContext for authenticated requests (e.g. CMS preview)

## 0.9.0

### Minor Changes

- ff69ffa: Replace FavoritesSession with unified ListsSession for product lists and favorites management

### Patch Changes

- ff69ffa: Add RequestContext for per-request locale/market overrides
